...
def blog():
    ...
    print(
        "  Qualquer adestramento de cachorro começa com os ensinamentos básicos. Um cão que não obedece a um mínimo de comandos não conseguirá aprender truques mais sofisticados, como saltar e pegar objetos específicos. Nesta matéria, ensinaremos como ensinar seu cachorrinho a sentar e deitar!!\n"
    )

    print("2- **Porque gatos não pegam carrapato normalmente?**\n")
    print(
        "  Uma coisa deve ficar clara: os gatos podem, sim, pegar carrapato! Não é que exista uma preferência do parasita pelo cachorro, mas sim pelo fato de os felinos serem, em geral, mais higiênicos. Então, o carrapato em gato é raro, mas não impossível!\n"
    )

    print(
        "3- **Linguagem corporal do cachorro: uma excelente forma de comunicação!**\n"
    )
    print(
        "  A linguagem corporal do cachorro é uma forma muito eficiente de comunicação. Por causa do extenso tempo de convívio com o ser humano, o cão consegue se fazer entender muito bem. Os cães têm uma forma única de comunicação. Eles expressam sentimentos e necessidades por meio de posturas, gestos, movimentos corporais e sons.\n"
    )
    ...
    if materia == 1:
        ...
        limpar_tela()
        print(
            "**Linguagem corporal do cachorro: uma excelente forma de comunicação!**\n"
        )
        print(
            "  A linguagem corporal do cachorro é uma forma muito eficiente de comunicação. Por causa do extenso tempo de convívio com o ser humano, o cão consegue se fazer entender muito bem. Os cães têm uma forma única de comunicação. Eles expressam sentimentos e necessidades por meio de posturas, gestos, movimentos corporais e sons.\n"
        )
        print(
            "  Saber interpretar a linguagem corporal canina é muito importante para que o tutor e o pet tenham uma boa convivência. Isso também contribui para o vínculo entre os familiares e outros animais da casa. A linguagem corporal do cachorro é uma forma de comunicação que vem evoluindo ao longo do tempo. Com ela, ele se comunica com os humanos e com outros animais. Ao observar atentamente um cachorro, é possível dizer como está o humor dele. A postura corporal e outros sinais revelam o estado emocional do pet .\n"
        )
        input("Pressione ENTER para voltar ao blog...\n")
        blog()
